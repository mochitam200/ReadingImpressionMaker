...
class ConsoleUI {
public:
    // 正しい読了日（YYMMDD/YYMM/YY+季節）が入力されるまで繰り返し入力を受け取り
    ...
    // 正しいタイトル（1〜50文字、全空白でない）が入力されるまで繰り返し読み込み
    static std::string inputTitle() {
        ...
        return ""; // 最後まで正しいタイトルを受け取れなかったので、空文字列を返す
    }

    // 正しい著者名（1〜50文字、全空白でない）が入力されるまで繰り返し読み込み
    static std::string inputAuthor() {
        ...
        return ""; // 最後まで正しい著者名受け取れなかったので、空文字列を返す
    }
